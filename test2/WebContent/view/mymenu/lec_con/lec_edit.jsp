...
<script>
	...
	$(function() {
		//<select id="status" class="val" name="status" number="${dtoJobInfo.status}">
		$('#keyword').ready(function(){
			var number = $(this).attr("number");
			$(this).find("option").each(function(){
				if($(this).val()==number)$(this).prop("selected", true);
			});
		});
...
<body>
	...
	<hr class="bar"/>
		...
		<span class="navigator deps">&nbsp;&rsaquo;&nbsp;</span>
		<span class="navigator final">강의수정</span>
	...
	<div class ="detail ">
		<h1>[${dtoLecInfo.className}] 강의 수정</h1>
		<form method="post">
		<div class="bar_u"></div>
		<div class="column2_1">
			...
			<c:forEach items="${requestScope.roomlist }" var="beanRoom">
				<c:if test="${beanRoom.classno eq dtoLecInfo.classno}" >
					<option value="${beanRoom.classno}" selected="selected">${beanRoom.classno}</option> 
				</c:if>
				<c:if test="${beanRoom.classno ne dtoLecInfo.classno}" >
					<option value="${beanRoom.classno}">${beanRoom.classno}/${beanRoom.maxNum}명 제한</option> 
				</c:if>
			</c:forEach>
			</select></div>
			<div class="row"><label for="lecCode" class="lab">강의코드</label><span class="val auto">${dtoLecInfo.lecCode }</span><input id="lecCode" name="lecCode" type="hidden" value="${dtoLecInfo.lecCode }"/></div>
		</div>
		<div class="column2_1">
			<div class="row"><label for="openDate" class="lab">개강일</label><input id="openDate" type="date" name="openDate" class="val" value="${dtoLecInfo.openDate }"/></div>
			<div class="row"><label for="endDate" class="lab">종강일</label><input id="endDate" type="date" name="endDate" class="val" value="${dtoLecInfo.endDate }"/></div>
			...
			<div class="row"><label for="keyword" class="lab">키워드</label><select id="keyword" name="keyword" class="val" number="${dtoLecInfo.keyword}">
		...
		<div class="column2_1"> 
			<div class="row"><label for="preview" class="lab">강의미리보기</label><input type="text" id="preview" class="val long" name="preview" value="${dtoLecInfo.preView }" /></div>
		</div>
		<div class="column2_1">
			<div class="row"><label for="status" class="lab">공개여부</label><select id="status" class="val" name="status" number="${dtoLecInfo.status}">
		...
		<div class="bar_b"></div>
		<div class="contents"> 
			<textarea class="val" name="contents">${dtoLecInfo.contents }</textarea>
		</div>
		<div class="bar_b"></div>
